<template>
      <div id="search-contain">
            ...
            <div class="search-condition-contain">
                ...
                <!-- <beat-loader v-else :loading="loading" :color="'red'" :size="'15px'"></beat-loader> -->
            </div>
        </div>
</template>
...
export default {
    ...
    methods: {
        ...
        // Search videos by keywords
         searchVideos(query){
            this.axios.get(`${global.YOUTUBE_ROOT}/search?part=snippet&q=${query}&maxResults=${this.maxResults}&type=video&order=${this.filterSort}&safeSearch=${this.filterSafe}&key=${global.API_KEY}`)
            .then(res=>{
        ...
        searchMySubscriptions(){
            ...
            const recursiveSubscription = (nextPage=null) => {
                if(!nextPage){ // First Page
                    this.axios.get(`${global.YOUTUBE_ROOT}/subscriptions?part=snippet&mine=true&maxResults=50&key=${global.API_KEY}&access_token=${this.token}`)
                    .then(res=>{
                        ...
                        if(err.response.status == 401){
                            if(this.token){
                                this.emitError("Sign in expired. Need to re authenticate before getting subscriptions");
                                this.$router.push("Auth");
                            }else{
                                this.emitError("Need to authenticate before getting subscriptions");
                                this.$router.push("Auth");
                            }
                        }
                    });
                    ...
                    this.axios.get(`${global.YOUTUBE_ROOT}/subscriptions?part=snippet&mine=true&pageToken=${nextPage}&maxResults=50&key=${global.API_KEY}&access_token=${this.token}`)
                    .then(res=>{
                    ...
                    }).catch(err=>{ console.log("Error", err); recursiveSubscription(index+1); });
                }
            }
        ...
        massageSubscriptions(data){
            ...
            const recurseMassage = (index=0) => {
                 this.axios.get(`${global.YOUTUBE_ROOT}/search?part=snippet&channelId=${data[index].snippet.resourceId.channelId}&maxResults=5&type=video&order=date&safeSearch=${this.filterSafe}&key=${global.API_KEY}`)
                .then(res=>{
                    ...
                    if(index < data.length-1){
                        ...
                        videos.sort(function(a,b) {return (a.snippet.publishedAt < b.snippet.publishedAt) ? 1 : ((b.snippet.publishedAt < a.snippet.publishedAt) ? -1 : 0);} ); 
        ...
        searchChannels(query){
            this.searchQuery = "";
            this.axios.get(`${global.YOUTUBE_ROOT}/search?part=snippet&type=channel&q=${query}&order=${this.filterSort}&maxResults=20&key=${global.API_KEY}`)
            .then(res=>{
                console.log("Searched Res subscriptions", res.data);
                this.$store.commit("LOADING_OFF");
        ...
        searchPlaylist(query){
            this.searchQuery = "";
            this.axios.get(`${global.YOUTUBE_ROOT}/search?part=snippet&type=playlist&q=${query}&maxResults=20&key=${global.API_KEY}`)
            .then(res=>{
    ...
    computed: {
        ...mapState({ loading: "loading", token: "token", type: "type", maxResults: "maxResults", videos: "videos", filterSort: "filterSort", filterSafe: "filterSafe", filterDate: "filterDate", filterDuration: "filterDuration" }),
        placeholder(){
            switch (this.type){
                case 'my':
